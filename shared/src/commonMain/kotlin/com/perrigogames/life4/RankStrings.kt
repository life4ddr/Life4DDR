...
object RankStrings {
     ...
     */
    fun getSongSetString(
        clearType: ClearType,
        difficulties: IntArray,
    ): StringDesc {
     ...
     */
    fun getTrialCountString(
        rank: TrialRank,
        count: Int,
    ): StringDesc {
        return if (count == 1) {
            StringDesc.ResourceFormatted(MR.strings.rank_goal_clear_trial_single, StringDesc.ResourceFormatted(rank.nameRes))
        } else {
            StringDesc.ResourceFormatted(MR.strings.rank_goal_clear_trial, StringDesc.ResourceFormatted(rank.nameRes), count)
        }
    }
    ...
    fun folderString(folderCount: Int): StringDesc =
        when (folderCount) {
            1 ->
                StringDesc.ResourceFormatted(
    ...
    // the 1st Mix folder
    fun folderString(folderName: String): StringDesc =
        StringDesc.ResourceFormatted(
    ...
    // any 3 songs
    fun songCountString(songCount: Int): StringDesc =
        StringDesc.ResourceFormatted(
    ...
    fun clearString(
        ...
        clearLampString(clearType)
    } else {
        clearTypeString(clearType)
    }

    fun clearTypeString(clearType: ClearType): StringDesc =
        StringDesc.ResourceFormatted(
            when (clearType) {
    ...
    fun clearLampString(clearType: ClearType): StringDesc =
        StringDesc.ResourceFormatted(
            when (clearType) {
    ...
    fun songExceptionsModifier(
        groupString: StringDesc,
        songExceptions: List<StringDesc>,
    ): StringDesc =
        StringDesc.ResourceFormatted(
    ...
    fun higherDiffNumSuffix(allowsHigherDiffNum: Boolean): String =
        when (allowsHigherDiffNum) {
            true -> "+"
            false -> ""
        }
}
