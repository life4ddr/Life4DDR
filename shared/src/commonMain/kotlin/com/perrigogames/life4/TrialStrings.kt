...
object TrialStrings {
    fun scoreSingleSong(score: Int, song: String) = when (score) {
    ...
    fun scoreCountSongs(score: Int, count: Int) = when (score) {
    ...
    fun scoreCountOtherSongs(score: Int, count: Int) = when (score) {
    ...
    fun scoreEverySong(score: Int) = when (score) {
    ...
    fun scoreEveryOtherSong(score: Int) = when (score) {
    ...
    fun allowedBadJudgments(bad: Int) =
        if (bad == 0) "• " + StringDesc.ResourceFormatted(MR.strings.no_bad_judgments)
        else "• " + StringDesc.ResourceFormatted(MR.strings.bad_judgments_count, bad)

    fun allowedMissingExScore(bad: Int, total: Int?) = when {
    ...
    fun allowedTotalMisses(misses: Int) =
        if (misses == 0) "• " + StringDesc.ResourceFormatted(MR.strings.no_misses)
        else "• " + StringDesc.ResourceFormatted(MR.strings.misses_count, misses)

    fun allowedSongMisses(misses: Int) =
        if (misses == 0) "• " + StringDesc.ResourceFormatted(MR.strings.no_misses)
        else "• " + StringDesc.ResourceFormatted(MR.strings.misses_each_count, misses)

    fun clearFirstCountSongs(clearType: ClearType, songs: Int) =
        "• " + StringDesc.ResourceFormatted(MR.strings.clear_first_songs, StringDesc.ResourceFormatted(clearType.clearRes), songs)

    fun clearEverySong(clearType: ClearType) =
        "• " + StringDesc.ResourceFormatted(MR.strings.clear_every_song, StringDesc.ResourceFormatted(clearType.clearRes))

    fun clearTrial() = "• " + StringDesc.ResourceFormatted(MR.strings.pass_the_trial)
