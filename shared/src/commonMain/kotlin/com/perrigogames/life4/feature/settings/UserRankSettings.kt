...
class UserRankSettings : SettingsManager() {
    val rank: Flow<LadderRank?> =
        settings.getLongOrNullFlow(KEY_INFO_RANK)
            .map { LadderRank.parse(it) }

    private val _targetRank =
        settings.getLongOrNullFlow(KEY_INFO_TARGET_RANK)
            .map { LadderRank.parse(it) }
    ...
    val targetRank: Flow<LadderRank?> =
        combine(_targetRank, rank) { target, actual ->
            when {
                target != null -> target
